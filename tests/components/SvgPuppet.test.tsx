  ...
  members: [
    {
      ...
      isBehindParent: false,
    },
    {
      id: "child-member",
      name: "child",
      parentId: "root-member",
      children: [],
      isBehindParent: false,
    },
  ],
...
describe("SvgPuppetInlineSimple", () => {
  ...
  it("should fetch, process, and render the puppet", async () => {
    const { SvgPuppetInlineSimple } = await import(
      "../../src/components/SvgPuppet"
    );
    const onReady = vi.fn();
  ...
  it("should use the cache on second render", async () => {
    ...
    const { rerender } = render(
      <SvgPuppetInlineSimple src="test.svg" onReady={onReady1} />,
    );
