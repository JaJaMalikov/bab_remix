...
describe('FloatingPanel', () => {
  ...
  beforeEach(() => {
    vi.spyOn(useDraggable, 'useDraggable').mockReturnValue(mockUseDraggable as any);
    vi.spyOn(useResizable, 'useResizable').mockReturnValue(mockUseResizable as any);
  });
  ...
  it('should call onClose when the close button is clicked', () => {
    const onClose = vi.fn();
    render(
      <FloatingPanel title="Test Panel" initialPosition={{ x: 0, y: 0 }} onClose={onClose}>
        <div>Panel Content</div>
      </FloatingPanel>
    );
