...
describe('MenuBar', () => {
  ...
  beforeEach(() => {
    ...
    (projectSerializer.loadProjectFromFile as vi.Mock).mockResolvedValue({ scene: { items: [] }, tracks: [], duration: 0 });
  ...
  it('should call save function on save button click', async () => {
    ...
    await waitFor(() => expect(projectSerializer.serializeProject).toHaveBeenCalled());
    await waitFor(() => expect(projectSerializer.saveProjectToFile).toHaveBeenCalled());
  });
  ...
  it('should call load function on load button click', async () => {
    ...
    await waitFor(() => expect(projectSerializer.loadProjectFromFile).toHaveBeenCalled());
  });
